...
namespace PositivePsychologyAppHtml
{
	public class Fillword
	{
		...
		public List<string> NoGuessedWords;

	}

	public class FillWordWork
	{
		public Fillword GetFillword()
		{

			int MinimumWordLength = 6; // for a feelword
			...
			string pathToFile = System.IO.Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Dictionaries\\Rus\\wordswin.dct");
			FWD.AddFile(pathToFile);


			int FeelwordsCount = 1, FeelwordWidth = 25, FeelwordHeight = 19;
			int DeadCellLimit = 1;
			...
			var feelword = Feelwords[0];
			feelword.WordsShuffle();

			return FWInfoToList(feelword);

		}

		public Fillword FWInfoToList(FWInfo fwInfo)
		{
			Array[] result1 = new Array[19];
			for (int i = 0; i < 19; i++)
			{
				result1[i] = new Array[25];
			}

			var uu = result1[3];
			//uu[3] = "4";

			var result = new String[19, 25];
			// матрица
			for (int c = 0; c < fwInfo.Width; c++)
			{
				//ExcelCells.SetColumnWidth(c, 2.7);
				for (int r = 0; r < fwInfo.Height; r++)
				{
					FWCellInfo fwcell = fwInfo.Cells[c, r];

					switch (fwcell.State)
					{
						case FWCellStates.Empty:
							result[r, c] = "?";
							break;
						case FWCellStates.Dead:
							result[r, c] = " ";
							break;
						case FWCellStates.Char:
							result[r, c] = fwcell.Char.Symbol.ToString();
							break;
						default:
							throw new Exception("Unknown fwcell state");
					}
				}
			}

			// слова
			var words = fwInfo.WordsToString();

			var resultList = new List<string[]>();
			for (int i = 0; i < 19; i++)
			{
				resultList.Add(new String[25]);
			}
			...
			for (int i = 0; i < 19; i++)
			{
				for (int j = 0; j < 25; j++)
				{
					resultList[i][j] = result[i, j];
				}
			}

			var answer = new Fillword();
			answer.table = resultList;
			answer.words = words;
			answer.GuessedWords = fwInfo.Words;
	...
	public class FWDictionary : Dictionary<int, List<string>>
	{
		...
		/*public string GetRandomWordExact(int Length) // вернуть слово указанной длины
		{
		  if (!ContainsKey(Length))
			throw new Exception(string.Format("Have no words in the dictionary with a length: {0}", Length));
		  return this[Length][Rnd.Next(this[Length].Count)];
		}*/
		...
		/*
		/// <summary>Returns an unique list of words from the dictionary</summary>
		public List<string> GetUniqueList(int TotalLetters, int MinimumWordLength)
		{
		  var Result = new List<string>();
		  int CurrentLetters = 0;
		  int MaxLength = MaximumLength;
		  while (CurrentLetters < TotalLetters)
		  {
			string CurrentWord;
			int RemainLetters = TotalLetters - CurrentLetters;

			if (RemainLetters < MaximumLength)
			{
			  if (RemainLetters < MinimumWordLength)
			  {
				// delete last record and continue
				CurrentLetters -= Result[Result.Count - 1].Length;
				Result.RemoveAt(Result.Count - 1);
				continue;
			  }
			  else
				CurrentWord = GetRandomWordNotLess(RemainLetters);
			}
			else
			  CurrentWord = GetRandomWordNotLess(MinimumWordLength);

			if (!Result.Contains(CurrentWord))
			{
			  Result.Add(CurrentWord);
			  CurrentLetters += CurrentWord.Length;
			}
	...
	public class FWWordInfo : List<FWWordCharInfo>
	{
		...
		/*
		private static Dictionary<FWDirections, FWDirections[]> NextDirections = new Dictionary<FWDirections, FWDirections[]>()
		  {
			{ FWDirections.Left, new FWDirections[] { FWDirections.Up, FWDirections.Down } },
			{ FWDirections.Right, new FWDirections[] { FWDirections.Down, FWDirections.Up } },
			{ FWDirections.Up, new FWDirections[] { FWDirections.Left, FWDirections.Right } },
			{ FWDirections.Down, new FWDirections[] { FWDirections.Right, FWDirections.Left } }
		  };

		public static bool IsPossibleDirection(FWDirections Direction, ref int MaxUpSteps, ref int MaxDownSteps, ref int MaxLeftSteps, ref int MaxRightSteps)
		{
		  var Result = false;
		  switch (Direction)
		  { 
			case FWDirections.Up: 
			  if (MaxUpSteps > 0) { MaxUpSteps--; MaxDownSteps++; Result = true; } 
			  break;
			case FWDirections.Down:
			  if (MaxDownSteps > 0) { MaxDownSteps--; MaxUpSteps++; Result = true; }
			  break;
			case FWDirections.Left:
			  if (MaxLeftSteps > 0) { MaxLeftSteps--; MaxRightSteps++; Result = true; }
				break;
			case FWDirections.Right: 
			  if (MaxRightSteps > 0) { MaxRightSteps--; MaxLeftSteps++; Result = true; }
			  break;
			default:
		...
		public static FWDirections GetNextDirection(Random Rnd, FWDirections Current, double ChangeDirectionProbability, 
													ref int MaxUpSteps, ref int MaxDownSteps, ref int MaxLeftSteps, ref int MaxRightSteps)
		{
		  if (Current == FWDirections.Unknown || Current == FWDirections.Last) throw new ArgumentException("Invalid value", "Current");

		  var NextDirection = Current; // by default, do not change direction
		  while (true)
		  {
			var CurrentDirectionProbability = Rnd.NextDouble();
			if (CurrentDirectionProbability <= ChangeDirectionProbability) // изменяем направление
			{
			  var PossibleDirections = NextDirections[Current];
			  var SelectionProbability = (double)1.0 / (double)PossibleDirections.Length; // вероятность выбора того или иного пути
			  int Index = Convert.ToInt32(Math.Truncate(Rnd.NextDouble() / SelectionProbability));
			  NextDirection = PossibleDirections[Index];
			  NextDirection = FWDirections.Up;
			}
			if (IsPossibleDirection(NextDirection, ref MaxUpSteps, ref MaxDownSteps, ref MaxLeftSteps, ref MaxRightSteps)) break;
		  }
